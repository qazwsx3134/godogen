...
| 先做 | 重做對話框與常用元件的視覺：嘗試底部滿版、半透明對話層、上緣分隔線，核對名牌、選項、懸浮鈕和窄螢幕可讀性。需求原文與圖片見[UI 備忘](../../prototypes/debt-commission/TODO.md)及[參考圖](../../prototypes/debt-commission/example/image.jpg) | 先做一版可比較的 Web 畫面，再以手機試玩決定是否取代目前 28% 對話框設計；若採用，回寫 Roadmap 的版面基準 |
...
- Safe Area 目前驗到瀏覽器避開系統安全區的情況；全螢幕或 PWA 顯示尚未驗。正式美術、音樂與 UI 風格未定，現有立繪與背景是 placeholder。
