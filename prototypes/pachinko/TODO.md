...
1. **釘微調的 20 組分布**（本來排 M4）。旋鈕已經找到、校準工具在手上，很便宜。
2. **重做接管中段的運動模型**（M1 留下的債，M3 的第一件事）。
3. **回報實機的音訊與觸控判讀**——M1 僅剩的三個未結出場條件，只有人能判。
...
- [ ] **釘微調的 20 組分布**（[ADR 0006](docs/adr/0006-nails-are-reset-each-session.md)）。
      旋鈕是誘導釘 V 底開口，90–102 對應入賞率 10–16；`tools/measure_start_rate.gd`
      已經支援 `--guide=` / `--mouth=` 掃描
- [ ] `sim.gd` 加物理類的分布檢查：20 組全數落在 9.5–16.5、無離群
...
- **誘導牆的寬度是懸崖不是旋鈕**（260 給 17.4 発/轉，275 給 5.8）。它只負責
  「確實接得到球」，校準交給平滑的那個參數。整個 10–16 的範圍只要 ±6% 的開口變動。
