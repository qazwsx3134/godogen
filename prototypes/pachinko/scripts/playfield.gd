...
## 也就是 ADR 0006 的釘調整範圍 10–15.5 只需要 ±6% 的開口變動——那正是
## 「肉眼看不出來的細微變動」該有的量級。
var guide_mouth := 96.0
## 電チュー的寬度。確變中右打的入賞率旋鈕，目標每 4 発一轉。
var gate_width := 110.0

## 釘調整（[ADR 0006]）。`-1` ＝**固定標準台，不抽**，而這是預設值：
## 驗收必須可重複，否則「這版演出變差」和「這次抽到辛台」分不開。
## 一場一台，開新場才給 seed。
var nail_seed := -1
## 抽出來的分布：常態，夾在校準曲線量過的範圍內。用常態而不是均勻，是因為真機的
## 店家多數台維持在標準附近，甘台辛台是少數。
##
## 中心 95 而不是 96，因為入賞率對開口是**凸函數**（96→99 漲 1.9，93→96 只漲 0.7），
## 所以對稱地抽開口會得到偏辛的平均值。第一次量到平均 12.56，就是這個原因。
##
## 辛端夾在 101 是**推導出來的，不是喜好**：SP ≥ 3 次需要基線 ≥ 3÷(4/319) = 239.3 轉，
## 也就是 3000÷(入賞率−3) ≥ 239.3，解出入賞率 ≤ 15.54 発/轉，對應開口約 101。
## ADR 0006 寫的範圍上限 16 其實只給 2.90 次，破自己的底線。
...
func _build() -> void:
	# 釘調整只動誘導釘的開口，不碰抽選。大當機率永遠是 1/319，它改變的只是
	# 同樣的球數能買到幾次抽選（ADR 0006）。
	if nail_seed >= 0:
